...
PKG_NAME := $(shell echo $(SW_NAME) | tr A-Z a-z)
PKG_BUILD := $(shell grep '"O_PKG_VER"' $(SPEC)       | sed -e 's/^.*\s*:\s*\"\(.*\)\".*$$/\1/' )

#FILES_MASK := $(SW_NAME)-$(SW_VER)-Setup.exe
#  		WinSCP-5.13.9-Setup.exe

INST_TYPE := $(shell grep '"O_USE_INSTALLER"' $(SPEC) | cut -d ":" -f 2 | cut -d '"' -f 2)

ifeq ($(INST_TYPE),INNO)
FILES_MASK := $(SW_NAME)-$(SW_VER)-Setup.exe
GREP_MASK := $(SW_NAME)-$(SW_VER)-Setup.exe
else ifeq ($(INST_TYPE),MSI)
FILES_MASK := $(SW_NAME)-$(SW_VER).msi
GREP_MASK := $(SW_NAME)-$(SW_VER).msi
else
FILES_MASK := $(SW_NAME)-$(SW_VER)*.*
GREP_MASK := $(SW_NAME)-$(SW_VER)
endif
FILES_EXPECTED = 1
...
copy_from_src:	build_dirs build_md5
	...
	@cp -upL $(SRC_DIR)/CLIENT_DATA/*.inf  $(BUILD_DIR)/CLIENT_DATA/
